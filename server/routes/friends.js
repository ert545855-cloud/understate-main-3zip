...
// Block
router.post('/block', authMiddleware, asyncHandler(async (req, res) => {
  const { username } = req.body;
...
router.delete('/block/:blockedId', authMiddleware, asyncHandler(async (req, res) => {
  await friendSvc.unblockUser(req.user.id, req.params.blockedId);
  res.json({ success: true });
}));

router.get('/blocks', authMiddleware, asyncHandler(async (req, res) => {
